...
class Solver:
  def __init__(self, data, n_blocks, weights, exclude_presenters):
    ...
    self.exclude_presenters_indeces = set({self.person_to_index[person_name] for person_name in self.exclude_presenters})

    self.problem_solved = False
    self.problem = LpProblem("learning-experiment-minimum-cost-schedule", LpMinimize)
    self.x = {(i,j,k): LpVariable(name=f"x[{i},{j},{k}]", cat=LpInteger, lowBound=0, upBound=1) for i in range(self.n_people) for j in set(range(self.n_people)).difference(set({i})).difference(self.exclude_presenters_indeces) for k in range(self.n_blocks)}
  ...
  def _add_constraints(self):
    ...
    for i in set(range(self.n_people)).difference(self.exclude_presenters_indeces):
      self.problem += lpSum(self.x[i,j,k] for j in set(range(self.n_people)).difference(set({i})).difference(self.exclude_presenters_indeces) for k in range(self.n_blocks)) == 1

    # each person either presents once or listens once in each block
    for i in set(range(self.n_people)).difference(self.exclude_presenters_indeces):
      for k in range(self.n_blocks):
        present_times = lpSum(self.x[j,i,k] for j in set(range(self.n_people)).difference(set({i})))
        listen_times = lpSum(self.x[i,j,k] for j in set(range(self.n_people)).difference(set({i})).difference(self.exclude_presenters_indeces))
    ...
    for i in self.exclude_presenters_indeces:
      for k in range(self.n_blocks):
        self.problem += lpSum(self.x[i,j,k] for j in set(range(self.n_people)).difference(set({i})).difference(self.exclude_presenters_indeces)) == 1
    
    # each block has exactly n/2 presentations
    for k in range(self.n_blocks):
      self.problem += lpSum(self.x[i,j,k] for i in range(self.n_people) for j in set(range(self.n_people)).difference(set({i})).difference(self.exclude_presenters_indeces)) == ceil(self.n_people/2)
  ...
  def _add_objective(self, data):
    ...
    self.problem += lpSum(self.x[i,j,k]*self.edge_costs[i,j] for i in range(self.n_people) for j in set(range(self.n_people)).difference(set({i})).difference(self.exclude_presenters_indeces) for k in range(self.n_blocks))
  ...
  def print_results(self, print_preferences=False):
